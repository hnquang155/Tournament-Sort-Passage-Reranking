...
class SetwiseLlmRanker(LlmRanker):
    ...
    def compare(self, query: str, docs: List):
        ...
        if self.scoring == 'generation':
            ...
            if self.config.model_type == 't5' or self.config.model_type == 'llama':
                input_ids = self.tokenizer(input_text, return_tensors="pt").input_ids.to(self.device)
                self.total_prompt_tokens += input_ids.shape[1]
    ...
    def score_passage(self, passage: str) -> float:
        # Replace this with the actual scoring function
        return int(passage.split()[-1])


    def build_tournament_tree(self, query, docs: List[str], r: int = 1, cached_group = [], cached_rank_index = []):
        """
        Build a tournament tree for ranking passages.
        """
        n = len(docs)
        tree = []

        # Create leaf nodes
        for i in range(0, n, self.num_child):
            temp = 0
            group = docs[i:i + self.num_child]
            while temp < r:
                #print("Temp: ", temp)
                if group == []:
                    temp += 1
                    continue
                if group not in cached_group:
                    top_r = self.compare(query, group)
                    best_ind = self.CHARACTERS.index(top_r)

                    # cache the ranking
                    cached_group.append(copy.deepcopy(group))
                    cached_rank_index.append(best_ind)
                    #print("Len group: ", len(group))

                    tree.append(group[best_ind])
                    group.remove(group[best_ind])
                    temp += 1
                else:
                    #print(" === Cached === ")
                    idx_in_cached  = cached_group.index(group)

                        # print(idx_in_cached)
                        # print(cached_rank_index[idx_in_cached])
                        # print(len(group)-1 < cached_rank_index[idx_in_cached])
                    tree.append(group[cached_rank_index[idx_in_cached]])
                    group.remove(group[cached_rank_index[idx_in_cached]])
                    temp += 1
        ...
        while len(tree) > r:
            next_level = []
            for i in range(0, len(tree),  self.num_child):
                temp = 0
                group = tree[i:i+ self.num_child]
                while temp < r:
                    if group == []:
                        temp += 1
                        continue
                    if group not in cached_group:
                        ...
                        cached_group.append(group.copy())
                        cached_rank_index.append(best_ind)
                        
                        # print(len(group)-1 < best_ind)
                        # print(len(group), best_ind)
                        # print(len(cached_group), len(cached_rank_index))
                        if group[best_ind] not in next_level:
                            next_level.append(group[best_ind])
                        group.remove(group[best_ind])
                        # print(group)
                        # print(group in cached_group)
                    else:
                        # print(len(cached_group) == len(cached_rank_index))
                        # print(group)
                        ...
                        # print(cached_rank_index[idx_in_cached])
                        # print(len(group)-1 < cached_rank_index[idx_in_cached])
                        if group[cached_rank_index[idx_in_cached]] not in next_level:
                            next_level.append(group[cached_rank_index[idx_in_cached]])
                        group.remove(group[cached_rank_index[idx_in_cached]])
                    temp += 1
            tree = next_level

        return tree, cached_group, cached_rank_index

    def extract_top(self, tree) -> List[str]:
        """
        Extract the top passages from the tournament tree.
        """
        return tree[0]
    # Example Usage
    ...
    def rerank(self,  query: str, ranking: List[SearchResult]) -> List[SearchResult]:
        ...
        ##  this is a bit slower but standard bobblesort implementation, keep here FYI
            ...
            temp_list = copy.deepcopy(ranking)
            k_rank = []
            cached_group = []
            cached_rank_index = []
            while len(k_rank)<self.k:
                # Build the tournament tree
                tree,  cached_group, cached_rank_index = self.build_tournament_tree(query, ranking, r=self.r_tounament, cached_group= cached_group, cached_rank_index=cached_rank_index)

                # Extract the top-k passages
                best_passage = self.extract_top(tree)

                print(best_passage)
                #print("Top Passages:", best_passage, next_best_passage)
                best_passage_idx = ranking.index(best_passage)
                if best_passage not in k_rank:
                    k_rank.append(best_passage)
                try:
                    temp_list = [passage for passage in temp_list if passage != best_passage]
                    print("Removed")
                except:
                    pass
            
                rand = random.choice(temp_list)
                ranking = [rand if passage == best_passage else passage for passage in ranking]
            
            
            ranking[:self.k] = k_rank[:self.k]
